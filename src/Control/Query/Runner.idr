...
export covering
engine : QIface c => DebugFlag => PrimRunner s c -> F1 s (Engine s c)
...
parameters (0 c     : QTypes)
           ...
           {auto df : DebugFlag}
           {auto qi : QIface c}
           {auto en : Enum (Query c) qi.nqueries}
           (curr    : Version)
           (store   : QStore s c)
  ...
  verify q arg (QS r c v ds) t =
   let _ # t := debugIf1 "verifying; \{logArg q arg}; current: \{curr}; query: \{v}; changed: \{c}" t
    in case v == curr of
         True  => True # t
  ...
  verifyDeps v (QK q arg :: ks) t =
   let _ # t := debugIf1 "verifying dependency; \{logArg q arg}" t
    in case lkp store q arg t of
         Nothing # t => False # t
         Just qs # t => case qs.changed > v of
           True  => False # t
  ...
  compute : ERunner s c
  compute q arg t =
   let _   # t := debugIf1 "computing query; \{logArg q arg}" t
       sk  # t := read1 stack t
       _   # t := write1 stack [<] t
       r   # t := toResult (qr q arg t)
       skr # t := read1 stack t
       _   # t := mem store q arg (QS r curr curr $ skr <>> []) t
       _   # t := debugIf1 "memoizing query; \{logArg q arg}" t
       _   # t := debugIf1 "dependencies for \{logArg q arg}: \{logDeps skr}" t
  ...
  loop : ERunner s c
  loop q arg t =
   let _ # t := debugIf1 "looking up query; \{logArg q arg}" t
    in case lkp store q arg t of
         Nothing  # t =>
          let _ # t := debugIf1 "query not memoized; \{logArg q arg}" t
           in compute q arg t
         Just qst # t => case verify q arg qst t of
           False # t => compute q arg t
           True  # t =>
            let _ # t := mod1 stack (:< QK q arg) t
             in resultToERes qst.result t
...
parameters {auto qi : QIface c}
  ...
  covering %inline
  runner : PrimRunner s c -> FRunner s c
  runner prim q arg t =
   let en         := qi.qen
       v      # t := read1 ref t
